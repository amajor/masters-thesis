...
\note{
  \begin{itemize}
    \item When we build new software systems, there are several areas of concern.
    \item Namely, COST, TIME-TO-DELIVER, and also, often to a lesser extent, QUALITY.
    \item Though quality can be a factor we consider when planning a project, it is a hard attribute to understand and measure.
    \item There are a number of tools available, some built into IDEs and others as 3rd party linter tools.
    \item Two such tools are Pylint and Radon, which can be used to analyze Python projects.
    \item In particular, we will focus on the refactor violations that are noted in these code reports, as these warnings can lead us to code smells.
  \end{itemize}
}
...
\note{
  \begin{itemize}
    ...
    \item However, if users are engaged and actively using software, changes are they are going to push it to its limits and want new features.
    \item Over time, there can also be new security threats and new laws that require software to be updated.
    \item These types of driving forces require a community of engaged users and in repsect to open source software, a community of engaged developers.
  \end{itemize}
}
...
\note{
  \begin{itemize}
    ...
    \begin{itemize}
      \item Naming
      \item Small methods
      \item Organized files
    \end{itemize}
    \item Studies have shown that a large project of a typical project's cost lies not in the development phase, but in the maintenance phase!
  \end{itemize}
}
...
\note{
  \begin{itemize}
    ...
    \item We should also remember other good practices, like low couping and high cohesion.
    \begin{itemize}
      \item Remember, coupling refers to the degree to which different modules depend on each other (we want modules to be independent from each other)
    ...
    \item And having condience around the metrics that tools like Pylint and Radon can offer should be able to help us keep our systems maintainable.
  \end{itemize}
}
...
\note{
  \begin{itemize}
    ...
    \item This ``historical record'' can help developers in the open source community understand the code.
...
\note{
  \begin{itemize}
    ...
    \item As we review open source systems, finding classes with frequent changes generally mean one of two things\dots
    \begin{itemize}
      \item the class is easy to extend (which is good!)
      \item or the class is highly coupled to other classes (code smell!)
    \end{itemize}
...
\note{
  \begin{itemize}
    ...
    \item For a very simplified view, we averaged the Maintainability Index of each project file in a repository, giving us a single number for general comparison with each repository.
    \item In Radon's grading system, any value over 20 is considered ``very maintainable''.
...
\note{
  \begin{itemize}
    \item With some of this information, we can now get an idea of where these projects stand in regards to their refactor message ratios compared to each other.
    ...
    \item This may be a impacted by the data set we chose; perhaps projects with highly engaged communities tend to keep their code in a maintainable fashion, because this would be the only way for many people to be involved.
  \end{itemize}
}
...
\note{
  \begin{itemize}
    ...
    \item The one exception is our ``worst'' repository, raven-python, which was deprecated, but in exchange for an improved system.
    ...
    \item Open source systems, and any project with many contributors, can be very vulnerable to degrading quality. It's the nature of change over time.
...
\note{
  \begin{itemize}
    ...
    \item What we've found is a reinforcement of what we already know.
    ...
    \item Regardless of your code base, understand what set of standards will work best for your team.
    \item Then auto-enforce these standards in your pipeline. This keeps everyone honest and will imporve the ability to evolve your project.
